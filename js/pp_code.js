	...
	*/
	...
	$('#destinationHotels').on("input",function(){
		if ($('#destinationHotels').val().length > 3) {
			...
			var _url = 'https://pp.travelrepublic.co.uk/services/autocompleter.svc/GetHotelDestinations?'+_data;
			jQuery.ajax({
				url: _url,
			    dataType: 'json',
	...
	$('#btnSearchHotel').on("vclick",function(){
		...
		var _url = 'https://pp.travelrepublic.co.uk/include/handlers/HotelSearch.ashx?nc=26430';
		...
		jQuery.ajax({
		    ...
		    success: function(data, textStatus, jqXHR) {
		        // Calls Success. If data found on the service then it would be inside "DATA" variable
			   	...
			   	var _url = "https://pp.travelrepublic.co.uk/hotels/availability/HotelAvailabilityOptions.ashx?" + _param;
			   	console.log(_url);		   
			   	jQuery.ajax({
			   		dataType: "json",
			   		headers: { "Content-Type": "application/json", "Accept": "application/json" },
	...
	$('#availableHotels').delegate('a','vclick',function(){
		...
		if(typeof estabId === "undefined"){
			...
			var _url = "https://m.pp.travelrepublic.co.uk/Include/Handlers/HotelSearchHandler.aspx/GetRoomsData";			
			var formData = {"estabId":estabId,"hguid":searchId};		
			...
			jQuery.ajax({
				...
				success: function(data, textStatus, jqXHR) {
					...
					//alert(JSON.parse(data.d).rooms[0].roomDesc);
					
					var roomData = JSON.parse(data.d);
					for(var i in roomData.rooms){
				    	//availableRooms;
				    	roomId = roomData.rooms[i].id;
				    	roomDescription = roomData.rooms[i].roomDesc;
				    	boardType = roomData.rooms[i].board;
				    	costPerNight = roomData.rooms[i].roomCost;
